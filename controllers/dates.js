...
router.post('/api/dentalClinics/:dentalClinicId/dates',function(req, res, next) {
    var date = new Date({
        date: req.body.date,
...
router.get('/api/dentalClinics/:dentalClinicId/dates', function (req, res, next) {
    ...
    Date.find({dentalClinic: {$eq:dentalClinic_id}}, function(err,date) {
        if (err) { return next(err); }
        if(date === null) {
            return res.status(404).json({'message' : 'There are no dates for this denal clinic!'});
        }
